...
// insert the lower nbits bits from w32 into accumulator, update counter, accumulator
//   [L|B]E64_STREAM_INSERT_NBITS(stream, w32, nbits)
//   [L|B]E64_EZ_INSERT_NBITS(w32, nbits)
...
//   [L|B]E64_STREAM_PUT_NBITS(stream, w32, nbits)
//   [L|B]E64_EZ_PUT_NBITS(w32, nbits)
// align insertion point to a 32 bit boundary
//   [L|B]E64_STREAM_INSERT_ALIGN(stream)
//   [L|B]E64_EZ_INSERT_ALIGN
//
// N.B. : if w32 and accumulator are signed variables, the extract will produce a "signed" result
//        if w32 and accumulator are unsigned variables, the extract will produce an "unsigned" result
// initialize stream for extraction
//   [L|B]E64_STREAM_XTRACT_BEGIN(stream)
//   [L|B]E64_EZ_XTRACT_BEGIN
// take a peek at nbits bits from accumulator into w32
//   [L|B]E64_STREAM_PEEK_NBITS(stream, w32, nbits)
//   [L|B]E64_STREAM_PEEK_NBITS_S(stream, w32, nbits)
//   [L|B]E64_EZ_PEEK_NBITS(w32, nbits)
//   [L|B]E64_EZ_PEEK_NBITS_S(w32, nbits)
// extract nbits bits into w32 from accumulator, update xtract, accumulator
//   [L|B]E64_STREAM_XTRACT_NBITS(stream, w32, nbits)
//   [L|B]E64_EZ_XTRACT_NBITS(w32, nbits)
...
//   [L|B]E64_STREAM_GET_NBITS(stream, w32, nbits)
//   [L|B]E64_EZ_GET_NBITS(w32, nbits)
...
CT_ASSERT_(sizeof(uint64_t) == 8)      // this better be true !
CT_ASSERT_(sizeof(void *) == 8)        // enforce 64 bit pointers
...
// insert the lower nbits bits from w32
...
// check that 32 bits can be safely inserted
// if not possible, store lower 32 bits of accumulator into stream
...
// store any residual data from accum into stream, fully updating control info
...
// combined INSERT_CHECK and INSERT_NBITS
...
// take a peek at nbits bits from stream
...
// check that 32 bits can be safely extracted
// if not possible, get extra 32 bits into from stream
...
// combined XTRACT_CHECK and XTRACT_NBITS
...
#define BE64_EZ_FAST_PUT_NBITS(w32, nbits)   BE64_FAST_PUT_NBITS(StReAm_acc_i, StReAm_insert, w32, nbits, StReAm_in)
...
// if not possible, get extra 32 bits into accum from stream
...
// finalize extraction, fully updating control info
