...
CT_ASSERT_(W32_SIZEOF(uint32_t) == 1)    // 1 x 32 bit item
...
CT_ASSERT(sizeof(wordstream) == 24, "wordstream size MUST be 24 bytes")    // 3 64 bit elements (1 x 64 bits + 4x 32 bits)
...
CT_ASSERT_(sizeof(wordstream_state) == 8)    // 2 32 bit elements
