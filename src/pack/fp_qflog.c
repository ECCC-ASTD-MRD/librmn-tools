...
// return fake integer
static inline int32_t fp_to_qflog_(float f, int nbits, float minabs, float zval){
...
int32_t fp_to_qflog_1(float f, int nbits, float minabs, float zval){
  return fp_to_qflog_(f, nbits, minabs, zval) ;
}
...
void fp_to_qflog(float *z, int32_t *q, int n, int32_t nbits, float minabs, float zval){
  ...
  nbits = (nbits < 0) ? 0 : nbits ;
  for(i=0 ; i<n ; i++){
    q[i] = fp_to_qflog_(z[i], nbits, minabs, zval) ;
  }
}
...
//         (MUST be the same value previously used by fp_to_qflog_s)
...
// q      [IN] : integer values
// n      [IN] : number of values
// nbits  [IN] : number of significant mantissa bits kept (MUST BE the same value used for fp_to_qflog_n)
...
// 0 comes back as 0.0f, -0.0f is never produced
static inline float qflog_to_fp_(int32_t i, int nbits, float minabs){
...
float qflog_to_fp_1(int32_t i, int nbits, float minabs){
  return qflog_to_fp_(i, nbits, minabs) ;
}
...
void qflog_to_fp(float *z, int32_t *q, int n, int32_t nbits, float minabs){
  ...
  for(i=0 ; i<n ; i++){
    z[i] = qflog_to_fp_(q[i], nbits, minabs) ;
  }
}
