...
module fortran_plugins
  ...
  contains
    ...
    procedure         :: function_pointer   => plugin_function_pointer     !< get function address
    !> \return true if valid library, false otherwise
    procedure         :: function_name   => plugin_function_symbol_name !< get function name
    !> \return number of symbols found in library, 0 if nothing found
    procedure         :: number_of_symbols => plugin_function_symbols     !< get number of advertized functions
  end type
...
!     if(associated(this % lib)) deallocate(this % lib)
  ...
  function plugin_function_symbol_name(this, ordinal) result(symbol_name)  !< get name of symbol #ordinal in this plugin
    implicit none
    class(plugin), intent(IN) :: this                          !< plugin type
    integer, intent(IN), value :: ordinal                      !< ordinal in advertized function table
    character(len=:), allocatable :: symbol_name             !< symbol (function) name
    logical :: status
    type(C_PTR) :: str
    integer :: lng
    character(C_CHAR), dimension(:), pointer :: temp
    ...
    if(C_ASSOCIATED(this % h) .and. ordinal >= 0) then
      ...
      lng = int( c_strlen_(str) )
      call c_f_pointer(str,temp,[lng])
      allocate( character(len=lng) :: symbol_name )
      symbol_name(1:lng) = transfer(temp(1:lng),symbol_name)
    else
      symbol_name = "Unknown"
    endif
