...
void array_lbounds_check(int low, int high){
  ...
  array_4d a4 = array_4d_zero, *ap4 = &a4 ;
  array_5d a5 = array_5d_zero, *ap5 = &a5 ;
  array_nd an = array_nd_zero, *apn = &an ;
  char *errmsg = "" ;
  ...
  fprintf(stdout,"sizeof(array_0d) = %ld, ", sizeof(array_0d)) ;
  fprintf(stdout,"sizeof(array_1d) = %ld, ", sizeof(array_1d)) ;
  fprintf(stdout,"sizeof(array_2d) = %ld, ", sizeof(array_2d)) ;
  fprintf(stdout,"sizeof(array_3d) = %ld, ", sizeof(array_3d)) ;
  fprintf(stdout,"sizeof(array_4d) = %ld, ", sizeof(array_4d)) ;
  fprintf(stdout,"sizeof(array_5d) = %ld, ", sizeof(array_5d)) ;
  fprintf(stdout,"sizeof(array_nd) = %ld\n", sizeof(array_nd)) ;

  for(uint32_t i=0 ; i<sizeof(printable_type)/sizeof(printable_type[0]) ; i++){
    fprintf(stdout,"data_code : %2d '%-7s', bit size = %3d\n", i, printable_type[i], size_of_type[i]) ;
  }
  print_meta(ap0, " : ap0 meta\n") ;
  print_meta(ap1, " : ap1 meta\n") ;
  print_meta(ap2, " : ap2 meta\n") ;
  print_meta(ap3, " : ap3 meta\n") ;
  print_meta(ap4, " : ap4 meta\n") ;
  print_meta(ap5, " : ap5 meta\n") ;
...
//   new_array(&a5, array_address(&a5), sizeof(int32_t), int_data, 8, 7, 6, 5, 4) ;
  fprintf(stdout, "after reshape 4, signature = %8.8x(%s), ", array_signature(&a5), array_no_data(&a5) ? "is empty" : "has data" ) ;
  print_dims(&a5, "\n") ;
  if(invalid_array(&a5)) goto fail ;
  ...
  print_flags(ap1, "ap1 flags : ") ;
  fprintf(stdout, "%12s array size is %6ld, subarray elements = %6d ", array_kind(ap1), array_bytes(ap1), subarray_dimension(ap1)) ; print_dims(ap1, "") ;
  fprintf(stdout, ", free_array(ap1) = %d\n", free_array(ap1)) ;
  ...
  print_flags(ap2, "ap2 flags : ") ;
  fprintf(stdout, "%12s array size is %6ld, subarray elements = %6d ", array_kind(ap2), array_bytes(ap2), subarray_dimension(ap2)) ; print_dims(ap2, "") ;
  fprintf(stdout, ", free_array(ap2) = %d\n", free_array(ap2)) ;
  ...
  print_flags(ap3, "ap3 flags : ") ;
  fprintf(stdout, "%12s array size is %6ld, subarray elements = %6d ", array_kind(ap3), array_bytes(ap3), subarray_dimension(ap3)) ; print_dims(ap3, "") ;
  fprintf(stdout, ", free_array(ap3) = %d\n", free_array(ap3)) ;
  ...
  print_flags(ap4, "ap4 flags : ") ;
  fprintf(stdout, "%12s array size is %6ld, subarray elements = %6d ", array_kind(ap4), array_bytes(ap4), subarray_dimension(ap4)) ; print_dims(ap4, "") ;
  fprintf(stdout, ", free_array(ap4) = %d\n", free_array(ap4)) ;
  ...
  print_flags(ap5, "ap5 flags : ") ;
  fprintf(stdout, "%12s array size is %6ld, subarray elements = %6d ", array_kind(ap5), array_bytes(ap5), subarray_dimension(ap5)) ; print_dims(ap5, "") ;
  fprintf(stdout, ", free_array(ap5) = %d\n", free_array(ap5)) ;
  ...
  print_flags(apn, "apn flags : ") ;
  fprintf(stdout, "%12s array size is %6ld, subarray elements = %6d ", array_kind(apn), array_bytes(apn), subarray_dimension(apn)) ; print_dims(apn, "") ;
  fprintf(stdout, ", free_array(apn) = %d\n", free_array(apn)) ;
...
void  print_strides(char *msg, __i32__5__ strides){
  int i ;
  fprintf(stdout, "%s strides : ", msg) ;
  for(i=0 ; i<5 ; i++){
    fprintf(stdout, "%d ", strides.i32[i]) ;
  }
  fprintf(stdout, "\n") ;
}
...
void print_bounds(array_nd *a1_, char *msg){
  ...
  int i ;
  fprintf(stdout, "%s bounds : ", msg) ;
  for(i=0 ; i<a1->rank ; i++){
    fprintf(stdout, "[g = %d:%d, l = %d:%d, lnn = %d] ",
...
void block_copy_check(int gni, int gnj, int gnk, int gnl, int gnm){
  ...
  (void)(gnl) ;
  (void)(gnm) ;
  array_1d a1 = array_1d_zero, b1 = array_1d_zero ;
  array_2d a2 = array_2d_zero, b2 = array_2d_zero ;
  array_3d a3 = array_3d_zero, b3 = array_3d_zero;
  array_4d a4 = array_4d_zero, b4 = array_4d_zero;
  array_5d a5 = array_5d_zero, b5 = array_5d_zero;
  ...
  fprintf(stdout, "a1 :") ; for(i=0 ; i<gni ; i++){ fprintf(stdout, " %8.8x", l1[i]) ;  } ; fprintf(stdout, "\n") ;
  ...
  fprintf(stdout, "t1 :") ; for(i=0 ; i<(int)sz1 ; i++){ fprintf(stdout, " %8.8x", t1[i]) ;  } ; fprintf(stdout, "\n") ;
  ...
  fprintf(stdout, "sz2 = %ld\n", sz2) ;
  if(sz2 > 0) goto fail ;
  fprintf(stdout, "b1 :") ; for(i=0 ; i<gni ; i++){ fprintf(stdout, " %8.8x", r1[i]) ;  } ; fprintf(stdout, "\n") ;
  ...
  fprintf(stdout, "a2[0][] :") ;
  for(i=0 ; i<gni ; i++){ fprintf(stdout, " %8.8x", l2[0][i] >> 12) ;  } ; fprintf(stdout, "\n") ;
  ...
  fprintf(stdout, "sz1 = %ld, expecting %d\n", sz1, (gni-1)*(gnj-1));
  ...
  fprintf(stdout, "a3[gnk-1][0][] :") ;
  for(i=0 ; i<gni ; i++){ fprintf(stdout, " %8.8x", l3[gnk-1][0][i] >> 8) ;  } ; fprintf(stdout, "\n") ;
  ...
  sz2 = subarray_set(&b3, (void *)t3, sz1 * sizeof(int32_t)) ;
  fprintf(stdout, "sz1 = %ld, expecting %d", sz1, (gni-3)*(gnj-3)*(gnk-3));
  fprintf(stdout, ", sz2 = %ld\n", sz2) ;
  diff = 0 ;
  for(k=0 ; k<gnk-3 ; k++){
    for(j=0 ; j<gnj-3 ; j++){
  ...
  fprintf(stdout, "a4[0][0][0][] :") ;
  for(i=0 ; i<gni ; i++){ fprintf(stdout, " %8.8x", l4[0][0][0][i] >> 4) ;  } ; fprintf(stdout, "\n") ;
  ...
  fprintf(stdout, "sz1 = %ld, expecting %d", sz1, gni*gnj*gnk*gnl);
  fprintf(stdout, ", sz2 = %ld\n", sz2) ;
  ...
  sz2 = subarray_set(&b4, (void *)t4, sz1 * sizeof(int32_t)) ;
  fprintf(stdout, "sz1 = %ld, expecting %d", sz1, gni*gnj*gnk*(gnl-1));
  fprintf(stdout, ", sz2 = %ld\n", sz2) ;
  diff = 0 ;
  for(l=0 ; l<gnl-1 ; l++){
    for(k=0 ; k<gnk-1 ; k++){
  ...
  fprintf(stdout, "a5[0][0][0][0][] :") ;
  for(i=0 ; i<gni ; i++){ fprintf(stdout, " %8.8x", l5[0][0][0][0][i]) ;  } ; fprintf(stdout, "\n") ;

  sz1 = subarray_get(&a5, (void *)t5, sizeof(t5), &bp) ;
  fprintf(stdout, "sz1 = %ld(%ld), sizeof(l5) = %ld, sizeof(t5) = %ld\n", sz1, sz1*sizeof(int32_t), sizeof(l5),sizeof(t5) ) ;
...
int main(int argc, char **argv){
  ...
  fprintf(stdout, "=============== error messages test ===============\n") ;
  array_1d a1 = array_1d_zero, *ap1 = &a1 ;
  array_2d a2 = array_2d_zero, *ap2 = &a2 ;
...
// return 0 ;
  fprintf(stdout, "=============== block copy test ===============\n") ;
  block_copy_check(9, 8, 7, 6, 5) ;
// goto end ;
  fprintf(stdout, "=============== array_lbounds test ===============\n") ;
  array_lbounds_check(1, 3);      // call bounds test, lower bound : 1, upper bound : 3
  fprintf(stdout, "SUCCESS\n") ;
// goto end ;
  fprintf(stdout, "=============== sub array test ===============\n") ;
//   array_1d a1 = array_1d_zero ;
//   array_2d a2 = array_2d_zero ;
  array_3d a3 = array_3d_zero ;
  array_3d b3 = array_3d_zero ;
  ...
  for(k=0 ; k<GNK-SUB ; k++){
    for(j=0 ; j<GNJ-SUB ; j++){
      for(i=0 ; i<GNI-SUB ; i++){
        ...
        if(*ptra != fijk(i, j, k)){
          fprintf(stdout, "[%3d,%3d,%3d], expected %8.8x, got %8.8x\n", i, j, k, fijk(i, j, k), *ptra) ;
          errors++ ;
          goto fail ;
        }
        // get block from a3
        ...
        if(subsize != 1000){
          fprintf(stdout, "subsize(get) = %ld, expected 1000\n", subsize) ;
          goto fail ;
        }
        // check block
        errsub = 0 ;
        ...
        if(0 != errsub){
          fprintf(stdout, "errsub(copy) = %d [%3d,%3d,%3d]\n", errsub, i, j, k) ;
          goto fail ;
        }
        // copy block into b3
        subsize = subarray_set(&b3, copy, sizeof(copy)) ;
        if(subsize != 1000){
          fprintf(stdout, "subsize(set) = %ld, expected 1000\n", subsize) ;
          goto fail ;
        }
        // check a3
        errsub = 0 ;
        ...
        errsub = subarray_check(SUB, SUB, SUB, copy,  i, SUB, j, SUB, k, SUB) ;
        if(SUB != errsub){
          fprintf(stdout, "errsub(copy) = %d, expected %d\n", errsub, SUB) ;
          goto fail ;
        }
        ...
        errsub = subarray_check(GNI, GNJ, GNK, (void *) ptra, i, SUB, j, SUB, k, SUB) ;
        if(SUB != errsub){
          fprintf(stdout, "errsub(ptra) = %d, expected %d [%3d,%3d,%3d]\n", errsub, SUB, i, j, k) ;
          goto fail ;
        }
...
fail:
  if(errors > 0) fprintf(stdout, "errors = %d\n", errors) ;
  fprintf(stdout, "FAILED\n") ;
  exit(1) ;
}
