...
//   uint64_t t[NTIMES] ;
  ...
  Lib_LogLevelNo(APP_LIBRMN, APP_INFO) ;
...
// goto timings ;
  ...
  TEE_PRINTF(TEE_INFO, "  expected        : maxs = %f, mins = %f, maxa = %f, mina = %f\n", ff[NP-1], ff[NP+1], -ff[NP+1], ff[NP]) ;
  TEE_PRINTF(TEE_INFO, "                  : allp = %d, allm = %d\n", W32_ALLP(l), W32_ALLM(l)) ;
  ...
  TEE_PRINTF(TEE_INFO, "SHORT MISSING SIMD: maxs = %f, mins = %f, maxa = %f, mina = %f\n", l.f.maxs, l.f.mins, l.f.maxa, l.f.mina) ;
...
//   l = IEEE32_extrema_missing(ff+NP-1, 3, ff+NP, 0, NULL) ;
  ...
  TEE_PRINTF(TEE_INFO, "NO MISSING    : maxs = %f, mins = %f, maxa = %f, mina = %f, min0 = %f (%d missing)\n", l.f.maxs, l.f.mins, l.f.maxa, l.f.mina, l.f.min0, l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected    : maxs = %f, mins = %f, maxa = %f, mina = %f, min0 = %f\n", ff[0], ff[NP2-1], -ff[NP2-1], 0.0, ff[NP]) ;
  ...
  TEE_PRINTF(TEE_INFO, "SIMD MISSING  : maxs = %f, mins = %f, maxa = %f, mina = %f, min0 = %f\n", l.f.maxs, l.f.mins, l.f.maxa, l.f.mina, l.f.min0) ;
...
//   l = IEEE32_extrema_missing(ff, NP2, ff+NP, 0, &fg) ;       // replacement value for "missing" values supplied
  TEE_PRINTF(TEE_INFO, "C    MISSING  : maxs = %f, mins = %f, maxa = %f, mina = %f, min0 = %f, missing = %f(n = %d)\n", l.f.maxs, l.f.mins, l.f.maxa, l.f.mina, l.f.min0, ff[NP], l.f.spec) ;
  ...
  TEE_PRINTF(TEE_INFO, "INT32_extrema : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d (%d missing)\n", l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected    : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n", fi[0], fi[NP2-1], -fi[NP2-1], 0, fi[NP]) ;
  if(l.i.maxs != fi[0] || l.i.mins != fi[NP2-1] || (int)l.i.maxa != -fi[NP2-1] || (int)l.i.mina != 0 || (int)l.i.min0 != fi[NP]) goto error ;

  l.i.maxa = l.i.mina = l.i.maxs = l.i.mins = 0 ;
  fi[1] = 0 ;
  fi[NP2-2] = 0 ;
  l = UINT32_extrema(fi, NP+1) ;
  TEE_PRINTF(TEE_INFO, "UINT32_extrema: maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d (%d missing)\n", l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected    : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n", 0, 1, fi[0], 0, fi[NP]) ;
  if(l.i.maxs != 0 || l.i.mins != +1 || (int)l.i.maxa != fi[0] || l.i.mina != 0 || (int)l.i.min0 != fi[NP]) goto error ;
...
//   l = INT32_extrema_missing(fi, NP2, fi+NP, 0, NULL) ;   // NO replacement value for "missing" values
  TEE_PRINTF(TEE_INFO, "I_extrema_mis : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d, missing = %d(n = %d)\n", l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, fi[NP], l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected    : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n", fi[0], fi[NP2-1], -fi[NP2-1], 0, fi[NP-1]) ;
  if(l.i.maxs != fi[0] || l.i.mins != fi[NP2-1] || (int)l.i.maxa != -fi[NP2-1] || l.i.mina != 0 || (int)l.i.min0 != fi[NP-1]) goto error ;
  TEE_PRINTF(TEE_INFO, "\n");
...
//   l = INT32_extrema_missing(fi, NP+1, NULL, 0, NULL) ;     // lower NP+1 values, no missing value
  TEE_PRINTF(TEE_INFO, "extrema(lo)    : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d (%d missing)\n", l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected     : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n", fi[0], 0, fi[0], 0, fi[NP]) ;
  if(l.i.maxs != fi[0] || l.i.mins != 0 || (int)l.i.maxa != fi[0] || l.i.mina != 0 || (int)l.i.min0 != fi[NP]) goto error ;
  TEE_PRINTF(TEE_INFO, "               : allp = %d, allm = %d, expected allp = %d, allm = %d\n", l.i.allp, l.i.allm, 1, 0) ;
...
//   l = UINT32_extrema_missing(fi, NP+1, NULL, 0, NULL) ;   // lower NP+1 values, no missing value
  TEE_PRINTF(TEE_INFO, "U_extrema_mis : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d (%d missing)\n", l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected    : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n", 0, 1, fi[0], 0, fi[NP]) ;
  if(l.i.maxs != 0 || l.i.mins != 1 || (int)l.i.maxa != fi[0] || l.i.mina != 0 || (int)l.i.min0 != fi[NP]) goto error ;
  TEE_PRINTF(TEE_INFO, "\n");
...
//   l = INT32_extrema_missing(fi, NP+1, fi+NP, 0, &iplug) ;     // lower NP+1 values, missing value = 1, plug = 2
  TEE_PRINTF(TEE_INFO, "extrema(lo)(m) : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d, missing = %d(n = %d), plug = %d\n",
                      l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, fi[NP], l.i.spec, iplug) ;
  TEE_PRINTF(TEE_INFO, "  expected     : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n",fi[0], 0, fi[0], 0, iplug) ;
  if(l.i.maxs != fi[0] || l.i.mins != 0 || (int)l.i.maxa != fi[0] || l.i.mina != 0 || (int)l.i.min0 != iplug) goto error ;
  TEE_PRINTF(TEE_INFO, "               : allp = %d, allm = %d, expected allp = %d, allm = %d\n", l.i.allp, l.i.allm, 1, 0) ;
  errmsg = "bad allp/allm values" ;
  if(l.i.allp != 1 || l.i.allm != 0) goto error ;
...
//   l = UINT32_extrema_missing(fi, NP+1, fi+NP, 0, &iplug) ;     // lower NP+1 values, missing value = 1, plug = 2
  TEE_PRINTF(TEE_INFO, "extrema(lo)(um): maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d, missing = %d(n = %d), plug = %d\n",
                      l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, fi[NP], l.i.spec, iplug) ;
  TEE_PRINTF(TEE_INFO, "  expected     : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n",0, 1, fi[0], 0, iplug) ;
  if(l.i.maxs != 0 || l.i.mins != 1 || (int)l.i.maxa != fi[0] || l.i.mina != 0 || (int)l.i.min0 != iplug) goto error ;
  TEE_PRINTF(TEE_INFO, "               : allp = %d, allm = %d, expected allp = %d, allm = %d\n", l.i.allp, l.i.allm, 1, 0) ;
  errmsg = "bad allp/allm values" ;
  if(l.i.allp != 1 || l.i.allm != 0) goto error ;
  TEE_PRINTF(TEE_INFO, "\n");
...
//   l = INT32_extrema_missing(fi+NP, NP+1, NULL, 0, NULL) ;  // upper NP+1 values, no missing value
  TEE_PRINTF(TEE_INFO, "extrema(hi)    : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d (%d missing)\n", l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, l.i.spec) ;
  TEE_PRINTF(TEE_INFO, "  expected     : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n",fi[NP], fi[NP2-1], -fi[NP2-1], 0, fi[NP]) ;
  if(l.i.maxs != fi[NP] || l.i.mins != fi[NP2-1] || (int)l.i.maxa != -fi[NP2-1] || l.i.mina != 0 || (int)l.i.min0 != fi[NP]) goto error ;
...
//   l = INT32_extrema_missing(fi+NP, NP+1, fi+NP, 0, &iplug) ;  // upper NP+1 values, missing value = 1
  TEE_PRINTF(TEE_INFO, "extrema(hi)(m) : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d, missing = %d(n = %d), plug = %d\n",
                      l.i.maxs, l.i.mins, l.i.maxa, l.i.mina, l.i.min0, fi[NP], l.i.spec, iplug) ;
  TEE_PRINTF(TEE_INFO, "  expected     : maxs = %d, mins = %d, maxa = %d, mina = %d, min0 = %d\n",0, fi[NP2-1], -fi[NP2-1], 0, -iplug) ;
  if(l.i.maxs != 0 || l.i.mins != fi[NP2-1] || (int)l.i.maxa != -fi[NP2-1] || l.i.mina != 0 || (int)l.i.min0 != -iplug) goto error ;
...
//   i = W32_replace_missing(ff, NP2, &fg, 0, &pad) ;
  TEE_PRINTF(TEE_INFO, "rep_missing   : %f, %f\n", ff[0], ff[NP2-1]) ;
  TEE_PRINTF(TEE_INFO, "  expected    : %f, %f\n", pad, pad) ;
...
//   TIME_LOOP_EZ(1000, NP2, l = IEEE32_extrema_missing(ff, NP2, ff+NP, 0, NULL)) ;
  TEE_PRINTF(TEE_INFO, "IEEE32_extrema_special     : %s\n",timer_msg);
//   TEE_PRINTF(TEE_INFO, "IEEE32_extrema_missing     : %s\n",timer_msg);

  TIME_LOOP_EZ(1000, NP2, l = INT32_extrema(fi, NP2)) ;
  TEE_PRINTF(TEE_INFO, "INT32_extrema              : %s\n",timer_msg);

  TIME_LOOP_EZ(1000, NP2, l = UINT32_extrema(fi, NP2)) ;
  TEE_PRINTF(TEE_INFO, "UINT32_extrema             : %s\n",timer_msg);

  s = (special_value) {fi+NP, NULL, 0u } ;
  TIME_LOOP_EZ(1000, NP2, l = INT32_extrema_special(fi, NP2, &s)) ;
//   TIME_LOOP_EZ(1000, NP2, l = INT32_extrema_missing(fi, NP2, fi+NP, 0, NULL)) ;
  TEE_PRINTF(TEE_INFO, "INT32_extrema_special      : %s\n",timer_msg);
//   TEE_PRINTF(TEE_INFO, "INT32_extrema_missing      : %s\n",timer_msg);
...
//   TIME_LOOP_EZ(1000, NP2, l = UINT32_extrema_missing(fi, NP2, fi+NP, 0, NULL)) ;
  TEE_PRINTF(TEE_INFO, "UINT32_extrema_special     : %s\n",timer_msg);
//   TEE_PRINTF(TEE_INFO, "UINT32_extrema_missing     : %s\n",timer_msg);

  s = (special_value) {&fg, &pad, 0u } ;
  TIME_LOOP_EZ(1000, NP2, i = W32_replace_special(ff, NP2, &s)) ;
//   TIME_LOOP_EZ(1000, NP2, i = W32_replace_missing(ff, NP2, &fg, 0, &pad)) ;
  TEE_PRINTF(TEE_INFO, "W32_replace_special        : %s\n",timer_msg);
//   TEE_PRINTF(TEE_INFO, "W32_replace_missing        : %s\n",timer_msg);

  if(timer_max == timer_min) TEE_PRINTF(TEE_INFO, "unlikely to happen\n");
  TEE_PRINTF(TEE_INFO, "\nSUCCESS\n") ;
  return 0 ;
