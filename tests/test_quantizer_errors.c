...
//       get_word_block(INDEX_F(f,i0,ni,j0), block0, in, ni, jn) ;
      move_mem32_block(INDEX_F(f,i0,ni,j0), in, block0, ni, ni, jn) ;
      // step 1 : quantize
...
//       put_word_block(INDEX_F(t,i0,ni,j0), block1, in, ni, jn) ;
      move_mem32_block(block1, ni, INDEX_F(t,i0,ni,j0), in, ni, jn) ;
...
//       get_word_block(f+i0+j0*ni, block0, in-i0, ni, jn-j0) ;
      move_mem32_block(f+i0+j0*ni, in-i0, block0, ni, ni, jn-j0) ;
//    alter block0
      for(j=0 ; j<jn-j0 ; j++){
        for(i=0 ; i<in-i0 ; i++){
...
//       put_word_block(t+i0+j0*ni, block0, in-i0, ni, jn-j0) ;
      move_mem32_block(block0, ni, t+i0+j0*ni, in-i0, ni, jn-j0) ;
#else
      for(j=j0 ; j<jn ; j++){
        for(i=i0 ; i<in ; i++){
        }
...
//       get_word_block(q + base, block, i1 - i0, ni, j1 - j0) ;     // get 8 x 8 block
      move_mem32_block(q + base, i1-i0, block, ni, ni, j1-j0) ;
...
//       get_word_block(f + base, fblk, nbpi, ni, nbpj) ;           // get block
      move_mem32_block(f + base, nbpi, fblk, ni, ni, nbpj) ;
...
//       put_word_block(g    + base, fblk, nbpi, ni, nbpj) ;        // put original block into g
      move_mem32_block(fblk, ni, g + base, nbpi, ni, nbpj) ;
      if(qbts[iblk] < 0) {
...
//       put_word_block(gblk + base, rblk, nbpi, ni, nbpj) ;  // put restored block into gblk
      move_mem32_block(rblk, ni, gblk + base, nbpi, ni, nbpj) ;
