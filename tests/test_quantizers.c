...
//       Z[j][i] = z[j][i] * ((z[j][i] < 0) ? (-z[j][i]) : z[j][i] ) ;
  ...
  if(fp32_exp(qzero) > e_base){
    e_range = fp32_exp(FLOAT_MAX_ABS(bp)) - fp32_exp(qzero) ;
    fprintf(stderr, "revised exponent range = %d\n", e_range) ;
  }
  int32_t e_bits = BitsNeeded_u32(e_range) ;
  fprintf(stderr, "%d bits needed for exponent\n", e_bits) ;
  for(j=0 ; j<NJ ; j++){
    for(i=0 ; i<NI ; i++){
//       q[j][i] = fp2q_log_(z[j][i], e_base, qzero, mbits, round);
    }
  }
...
//   adjust_block_properties(&bpi, int_data) ;
  ...
  uint32_t uq ;
  uq = fp2q_log_(FLOAT_MIN_ABS(bp), e_base, qzero, mbits, round) ;
  fprintf(stderr, "%8.8x = fp2q_log_(%f) [%f]\n", uq, FLOAT_MIN_ABS(bp), q2fp_log_(uq, e_base, mbits) ) ;

  uq = fp2q_log_(FLOAT_MAX_ABS(bp), e_base, qzero, mbits, round) ;
  fprintf(stderr, "%8.8x = fp2q_log_(%f) [%f]\n", uq, FLOAT_MAX_ABS(bp), q2fp_log_(uq, e_base, mbits) ) ;
