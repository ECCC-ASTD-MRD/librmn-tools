...
//   for(i=0 ; i<NPTS ; i++) array[i] = random() & MASK ;
  for(i=0 ; i<NPTS ; i++) {
    xrand = (random() & MASK) / 65536.0f ;
    array[i] = xrand * xrand * xrand * xrand * NPTS ;
  }
  for(i=0 ; i<6 ; i++) count[i] = 0 ;
  imax = 0 ;
  for(i=0 ; i<NPTS ; i++) imax = (array[i] > imax) ? array[i] : imax ;
  fprintf(stderr, "largest values is %d\n", imax) ;

  v_less_than(array, ref, count, NPTS) ;
  for(i=0 ; i<6 ; i++){
    int short_c = count[i] ;
